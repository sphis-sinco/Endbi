...
/**
 * Main gameplay state for the battle system.
 * Handles player/opponent logic, UI, and state transitions.
 */
class PlayState extends FlxState
{
	 ...
	 * @param player Player character name
	 * @param op Opponent character name (default: 'jujer')
	 */
	override public function new(player:String, op:String = 'jujer')
	{
		...
		ATTACK_SELECT_BUTTON = new FlxButton(0, FlxG.height - 3 * 48, 'Attack', () -> ATTACK_SELECT = true);
	...
	/**
	 * Sets up the battle scene, UI, and character positions.
	 */
	override public function create()
	{
		super.create();
		final charWidthOffset = 6, charHeightOffset = 6;
		...
		PLAYER_TEXT.size = OPPONENT_TEXT.size = 16;
		PLAYER_TEXT.color = OPPONENT_TEXT.color = 0x000000;
		add(PLAYER_TEXT);
		add(OPPONENT_TEXT);
		add(ATTACK_BUTTONS);
		ATTACK_BUTTONS.add(new FlxButton(0, 0, PLAYER.data.attack1.name, () -> attackOp(PLAYER.data.attack1)));
		ATTACK_BUTTONS.add(new FlxButton(0, 32, PLAYER.data.attack2.name, () -> attackOp(PLAYER.data.attack2)));
		ATTACK_BUTTONS.add(new FlxButton(0, 64, PLAYER.data.attack3.name, () -> attackOp(PLAYER.data.attack3)));
	...
	/**
	 * Handles player attack logic and triggers opponent response.
	...
	/**
	 * Handles logic when the opponent is defeated.
	 */
	...
	/**
	 * Handles player defence logic and triggers opponent response.
	 */
	...
	/**
	 * Handles opponent move logic and attack/defence decision.
	 * @param playerDefend Whether the player is defending
	 */
	public function opMove(playerDefend:Bool = false)
	{
		trace('------op-move------');
		var attack = false;
		attack = FlxG.random.bool(FlxG.random.int(0, 100))
			|| OPPONENT.LEVEL > PLAYER.LEVEL
			|| OPPONENT.ENERGY > PLAYER.ENERGY
			|| PLAYER.HP == 1
			|| OPPONENT_NEXT_MOVE == ATK_MOVE ? true : attack;
		attack = OPPONENT.ENERGY == 0 || OPPONENT_NEXT_MOVE == DEF_MOVE ? false : attack;
	...
	/**
	 * Handles logic when the player is defeated.
	 */
	...
	/**
	 * Main update loop for the battle state.
	 * Handles UI and state updates.
	 */
